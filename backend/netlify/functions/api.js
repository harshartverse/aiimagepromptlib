...
app.get('/stats', async (req, res) => {
  ...
  try {
    const statsRes = await db.execute({
      sql: `
        SELECT
          ...
          (SELECT COUNT(*) FROM prompts WHERE isNew = 1) AS newPrompts,
          (SELECT COUNT(*) FROM prompts WHERE isTrending = 1) AS trendingPrompts,
          (SELECT COUNT(*) FROM categories) AS totalCategories,
          (SELECT COUNT(*) FROM prompt_likes) AS totalLikes,
          (SELECT COUNT(*) FROM prompt_likes WHERE created_at >= ?) AS likesToday,
          (SELECT COUNT(*) FROM prompt_likes WHERE created_at >= ?) AS likes7Days,
          (SELECT COUNT(*) FROM prompt_likes WHERE created_at >= ?) AS likes30Days,
          (SELECT COUNT(*) FROM fcm_devices) AS registeredDevices,
          (SELECT COUNT(*) FROM fcm_devices WHERE created_at >= ?) AS newDevices7Days
      `,
      args: [startOfToday, sevenDaysAgo, thirtyDaysAgo, sevenDaysAgo]
    });

    const row = statsRes.rows[0] || {};

    // Category distribution query
    const catDistRes = await db.execute(`
      SELECT c.id, c.name AS categoryName, COUNT(p.id) AS promptCount
      FROM categories c
      LEFT JOIN prompts p ON p.category_id = c.id
      GROUP BY c.id
      ORDER BY promptCount DESC, c.name ASC
    `);

    const categoryDistribution = catDistRes.rows.map(r => ({
      id: String(r.id),
      categoryName: String(r.categoryName || 'Uncategorized'),
      promptCount: Number(r.promptCount || 0)
    }));

    // Top Liked Prompts query (Top 10)
    const topPromptsRes = await db.execute(`
      SELECT
        p.id, p.title, p.promptText, p.imageUrl, p.aiModel, p.gender, p.outputType,
    ...
    const topLikedPrompts = topPromptsRes.rows.map(r => ({
      id: String(r.id),
      title: String(r.title || ''),
      promptText: String(r.promptText || ''),
      imageUrl: String(r.imageUrl || ''),
      aiModel: String(r.aiModel || 'Gemini 2.0 Flash'),
      gender: String(r.gender || 'Male'),
      outputType: String(r.outputType || 'HQ 1:1'),
      categoryName: String(r.categoryName || 'Uncategorized'),
    ...
    res.json({
      ...
      newPrompts: Number(row.newPrompts || 0),
      trendingPrompts: Number(row.trendingPrompts || 0),
      totalCategories: Number(row.totalCategories || 0),
      totalLikes: Number(row.totalLikes || 0),
      likesToday: Number(row.likesToday || 0),
      likes7Days: Number(row.likes7Days || 0),
      likes30Days: Number(row.likes30Days || 0),
      registeredDevices: Number(row.registeredDevices || 0),
      newDevices7Days: Number(row.newDevices7Days || 0),
