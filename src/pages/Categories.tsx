...
import { Plus, Edit2, Trash2, Loader2, X, GripVertical, Save } from 'lucide-react';
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Manage prompt categories, taxonomy, and drag-and-drop display order</p>
        </div>
        <div className="flex items-center gap-3">
          {hasOrderChanged && (
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition-all shadow-lg shadow-emerald-900/30 animate-pulse"
            >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-900/50">
                <th className="p-4 text-sm font-medium text-gray-400 w-12 text-center">Order</th>
            ...
            <tbody>
              {categories.map((c, index) => (
                <tr
                  ...
                  className={`border-b border-gray-800 hover:bg-gray-800/50 transition-colors cursor-grab active:cursor-grabbing ${
                    draggedIndex === index ? 'bg-blue-900/30 border-blue-500/50' : ''
                  }`}
                >
                  <td className="p-4 text-center">
                    <GripVertical size={18} className="inline-block text-gray-500 hover:text-gray-300" />
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      {c.imageUrl ? (
